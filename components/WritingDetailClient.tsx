...
export default function WritingDetailClient({ writing, recentWritings }: WritingDetailClientProps) {
  return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            ...
            >
              {/* Header */}
              <div className="mb-8">
                <h1 className="text-2xl md:text-3xl font-light text-black mb-3">{writing.title}</h1>
                <p className="text-sm text-gray-500 font-light mb-4">{writing.description}</p>
                <div className="flex items-center gap-4 text-xs text-gray-400 font-light">
                  <span>{new Date(writing.createdAt).toLocaleDateString('ko-KR')}</span>
                  {writing.link && (
                    <>
                      <span>·</span>
                      <a
              ...
              {writing.images && writing.images.length > 0 && (
                <div className="mt-10">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {writing.images.map((image, idx) => (
                      <img
                        key={idx}
                        src={image}
                        alt={`${writing.title} - ${idx + 1}`}
                        className="w-full rounded-lg"
                      />
                    ))}
                  </div>
          ...
          <aside className="lg:col-span-1">
            <div className="sticky top-24">
              ...
              >
                <h3 className="text-sm font-medium mb-4">Recent Writings</h3>
